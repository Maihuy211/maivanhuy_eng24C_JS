...
function updateCart() {
    ...
    if (cart.length === 0) {
        cartContainer.innerHTML = '<span class="empty">Giỏ hàng trống</span>';
        document.getElementById("total").innerText = "Tổng: 0₫";
        return;
    }
    cart.forEach(item => {
        cartContainer.innerHTML += `
...
function addToCart(id) {
    ...
    if (cartIndex !== -1) {
        ...
        cart.push({ ...products.find(item => item.id === id), quantity: 1 });
    }
    updateCart();
}
...
function total() {
    ...
    document.getElementById("total").innerText = `Tổng: ${sum.toLocaleString('vi-VN')}₫`;
}
renderProducts();
